...
function WorkShop() {
  return (
    <>
      <Helmet>
        <title>
          Maruti Suzuki Authorized Service Center in Hyderabad | Saboo Maruti
        </title>
        <meta
          name='title'
          content='Maruti Suzuki Authorized Service Center in Hyderabad | Saboo Maruti'
        />
        <meta
          name='description'
          ...
          <div className='grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 px-3 md:px-0 gap-5'>
            {workshop.map((item, index) => (
              <div className='w-full  h-80 pt-1 px-4 text-left py-3 hover:text-white shadow-xl shadow-[#B0B0B0] group duration-500 border-t rounded-3xl relative overflow-hidden'>
                ...
                <div className='mb-2 mt-2 ml-3 text-2xl font-semibold'>
                  {item.address}
                </div>
                {/* <div className="mb-2">Adarsh Nagar, Hyderabad - 500063</div> */}
                <a href={item.map} target='_blank' rel='noreferrer'>
                  <div
                    className='flex justify-center duration-500 bg-center my-5 h-32 lg:h-60 group-hover:saturate-200 rounded-3xl grayscale group-hover:grayscale-0'
                    style={{
